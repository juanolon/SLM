...
class Diffusion(L.LightningModule):
  ...
  def forward(self, x, sigma, signal, stage="training"):
    """Returns log score."""
    ...
    with torch.cuda.amp.autocast(dtype=torch.float32):
      logits = self.backbone(x, sigma, signal)
  ...
  def _compute_loss(self, batch, prefix):
    ...
    if prefix == 'train':
      ...
      losses = self._valid_loss(batch, attention_mask)
    else:
  ...
  def validation_step(self, batch, batch_idx):
    ...
    def get_sei_profile(seq_one_hot):
      ...
      sei_out = sei(sei_inp).cpu().detach().float().numpy() # batchsize x 21,907
    ...
    seq_one_hot = batch[:, :, :4]
    signal = batch[:, :, 4:5]
    seq_pred = self._sample_newdiff(batch.size(0), signal, self.config.sampling.steps)
    seq_pred_one_hot = torch.nn.functional.one_hot(seq_pred, num_classes=self.vocab_size).float()

    if batch_idx not in self.sei_cache:
      sei_profile = get_sei_profile(seq_one_hot)
      self.sei_cache = sei_profile
    else:
      sei_profile = self.sei_cache[batch_idx]

    sei_profile_pred = get_sei_profile(seq_pred_one_hot)
    self.log('sp-mse', ((sei_profile - sei_profile_pred) ** 2).mean().item(), on_step=False, on_epoch=True, sync_dist=True)
  ...
  def on_validation_epoch_end(self):
    ...
    if self.config.mode == 'eval':
      loss = torch.stack(self.val_outputs['loss'])
      print(loss)
      loss = loss[~torch.isnan(loss)].mean()
      ppl = torch.exp(loss)
      self.log('val/ppl', ppl.item(), on_step=False, on_epoch=True, sync_dist=True)
  ...
  def _sample_newdiff(self, bsz, signal, numsteps):
    ...
    for i in range(1, numsteps + 1):
        ...
        t = 1 - t #reverse
        #   # [B,1,1]
        model_output = self.forward(x_t, t, signal, stage="inference") 
    ...
    predicted  = self.forward(x_t, t, signal, stage="inference")
    sample_pred = torch.argmax(predicted,dim=-1)

    return sample_pred


  def compute_reconstruction_loss(self, x0, signal, type="ce", stage="training") -> Tensor:
    bsz = x0.shape[0]
    ...
    output_params: Tensor = self.forward(net_inputs, t, signal, stage=stage)
    ...
    #         raise NotImplementedError
    if type=="ce":
      return self.bfn_loss.reconstruction_loss(x0, output_params, input_params).flatten(start_dim=1)
    elif type=="l2":
      ...
      return self.bfn_loss.cts_time_loss(x0, output_params.float(), input_params, t)
    else:
      raise NotImplementedError
  ...
  def _bfn_forward_loss(self, x0, signal, stage="training"):
    x0 = x0.argmax(-1)
  ...
  def _forward_new_diffusion(self,x0,stage="training"):
    ...
    model_output = self.forward(x_t, t, signal, stage=stage)
