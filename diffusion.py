...
def _sample_categorical(categorical_probs):
    gumbel_norm = (
        1e-10 - (torch.rand_like(categorical_probs, dtype=torch.float64) + 1e-10).log()
    )
...
class Diffusion(L.LightningModule):
    ...
    def forward(self, x, sigma):
        ...
        if self.parameterization == "dfm":
            return self._dfm_parameterization(logits=logits, xt=x)
        ...
        if (
            ...
            if self.trainer.global_rank == 0:
                ...
                sampled_table = wandb.Table(
                    columns=["Generated Samples"], data=[[s] for s in text_samples]
                )
