...
def _pad(tokenized, max_len, value, dim=2):
    ...
    # in case that longer sequences exists
    if(max_seq_len > max_len):
        max_len *= 2

    # cut off sequences that are too long
    if(max_seq_len > max_len):
        tokenized = [token[:max_len] for token in tokenized]
...
class OAMaskCollater(object):
    ...
    """
    def __init__(self, tokenizer=Tokenizer(), max_length=1024):
        self.tokenizer = tokenizer
        self.max_length = max_length


    def __call__(self, sequences):
        # print(f"len(sequences) for each collater: {len(sequences)}")
        # print(f"sequences[0]: {sequences[0]}")

        tokenized = [torch.tensor(self.tokenizer.encode(s)) for s in sequences]
        max_len = max(len(t) for t in tokenized)
        src=[]
        timesteps = []
        masks=[]
        mask_id = torch.tensor(self.tokenizer.mask_token_id, dtype=torch.int64)
        for i,x in enumerate(tokenized):
            ...
            src.append(x_t)
        # PAD out
        src = _pad(src, self.max_length, self.tokenizer.pad_token_id)
        masks = _pad(masks*1, self.max_length, 0) #, self.seq_length, 0)
