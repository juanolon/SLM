...
class Tokenizer(object):
    ...
    def __init__(self, protein_alphabet=MSA_ALPHABET, pad=MSA_PAD, mask=MASK, all_aas=MSA_AAS, gap=GAP, start=START,
        ...
        self.vocab_size = len(self.alphabet)    # ? either len(all_aas)
        ...
        self.eos_token = stop
        self.sep_token = sep
        self.padding_side, self.truncation_side = None, None
    ...
    def pad_token_id(self):
         return self.a_to_i[self.pad_token]

    @property
    def mask_token_id(self):
        return self.a_to_i[self.mask_token]

    @property
    def gap_token_id(self):
        return self.a_to_i[self.gap_token]

    @property
    def bos_token_id(self):
        return self.a_to_i[self.bos_token]

    @property
    def eos_token_id(self):
        return self.a_to_i[self.eos_token]

    @property
    def sep_token_id(self):
        return self.a_to_i[self.sep_token]
    ...
    def encode(self, seq):
        return np.array([self.a_to_i[a] for a in seq]) # for nested lists

    def tokenizeMSA(self, seq):
        return np.array([self.a_to_i[a] for a in seq]) # not nested

    def decode(self, x):
        return "".join([self.i_to_a[int(t.item())] for t in x]) # remove start and end tokens
    
    def batch_decode(self, batch_x):
        return [self.decode(x) for x in batch_x]
